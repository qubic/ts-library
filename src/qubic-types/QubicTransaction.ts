...
export class QubicTransaction implements IQubicBuildPackage {
  ...
  public payload: IQubicBuildPackage = new DynamicPayload(
    QubicDefinitions.MAX_TRANSACTION_SIZE
  );
  public signature: Signature = new Signature();

  public setSourcePublicKey(p: PublicKey | string): QubicTransaction {
    if (p instanceof PublicKey) this.sourcePublicKey = p;
    else this.sourcePublicKey = new PublicKey(p);
    return this;
  }
  public setDestinationPublicKey(p: PublicKey | string): QubicTransaction {
    if (p instanceof PublicKey) this.destinationPublicKey = p;
    else this.destinationPublicKey = new PublicKey(p);
    return this;
  }
  public setAmount(p: Long | number | bigint): QubicTransaction {
    if (p instanceof Long) this.amount = p;
    else this.amount = new Long(p);
    return this;
  }
  ...
  public setPayload(
    payload: DynamicPayload | IQubicBuildPackage
  ): QubicTransaction {
  ...
  private _internalSize() {
    return (
      this.sourcePublicKey.getPackageSize() +
